    ...
    return (
      <div className="todolist">
        {this.props.items.map(item => (
          <TodoItem key={uuid()} id={item.id} text={item.text} des={item.des} date={item.date} completed={item.done} onItemCompleted={this.props.onItemCompleted} onUpdateItem={this.props.onUpdateItem} onDeleteItem={this.props.onDeleteItem} />
        ))}
      </div>
    );
  }
...
class TodoItem extends Component {
  ...
  UpdateItem(event) {
    ...
    const Newupdate = {id:this.state.id,text: this.state.text,des:this.state.des,date:this.state.date}
      ...
      <div id="box" className={itemClass} ref={li => this._listItem = li}>
        ...
        <div className="col-md-9">
          {this.state.Isclick === true ?
            ...
            : <label className="form-check-label"><a onClick={this.handinfo} id="text_todo">{this.state.text}</a> <div className="duedate">{duedate}</div></label>}
        </div>
          ...
          <form className="submit_addtodo" onSubmit={this.UpdateItem}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <input type="date" className="form-control" onChange={this.handleDateUpdate} value={this.state.date} />
              </div>
            </div>
